...
class tally:
    def __init__(self,space_lim=tuple,space_n=int):
        ...
        self.mean = np.array([np.zeros(len(self.spacerange)-1) for __ in range(len(self.energyrange))])
        self.variance = np.array([np.zeros(len(self.spacerange)-1) for __ in range(len(self.energyrange))])
        self.iter = 0
    @property
    def avg(self):
        return self.mean
